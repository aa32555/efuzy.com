ZAA02GCOMM5 ;PG&A,ZAA02G-COMM,1.36,LOGON,DIALUP DISPATCH ROUTINE;12JUL96 3:08P;;;22OCT97  15:27 [ 02/17/98   9:40 AM ]
 ;Copyright (C) 1984, Patterson, Gray and Associates, Inc.
BEG S ZAA02GCOMMG=$S($D(^%ZAA02GSEND)#2:^%ZAA02GSEND,1:"^ZAA02GSEND")
 I $D(@ZAA02GCOMMG@("XLOGIN")) X ^("XLOGIN") Q:$D(DONE)
 S:'$D(ZAA02G) ZAA02G(1)=0 X @ZAA02G@("ECHO-OFF") R A:1 I A'=""!$T D FILTER C $I H 2 H
 I '$D(@ZAA02GCOMMG@("FILES")) G BEG1
 S $X=0 R "Do you want to Upload/Download Files via Kermit (Y or N) - ",A#1:20 G EXIT:A="" I "Yy"[A W A G SENDK
 G:"Nn"'[A EXIT W !!
BEG1 S $X=0 R "Enter Login Code - ",A#10:20 G EXIT:A=""  S @ZAA02GCOMMG@("LOG",+$H,$P($H,",",2))=A_$S($G(@ZAA02G@("OS"))="MSM":","_$ZDEV($I),1:"")
 S B=$S($D(@ZAA02GCOMMG@("LOGIN"))#2=0:"MUMPS",1:@ZAA02GCOMMG@("LOGIN")) S UP="ABCDEFGHIJKLMNOPQRSTUVWXYZ",LC="abcdefghijklmnopqrstuvwxyz",A=$TR(A,LC,UP) I A=$TR(B,LC,UP) G UCI
 I A="KERMIT" G SENDK
 I $D(@ZAA02GCOMMG@("PASSWD",A))#2 X ^(A) Q
 W "  Invalid Code" G EXIT
UCI X @ZAA02G@("ECHO-ON") I $D(@ZAA02GCOMMG@("LOGINROUTINE"))#2 G @^("LOGINROUTINE")
...
FILTER I $A(A)=$C(27) G PRINT
 ...
 R "",A:0,A:0,A:0 Q
LOG S Y="20\RHLS\3\ZAA02G-COMM LOGIN LOG",Y(0)="\EX\@ZAA02GCOMMG@(""LOG"")\$$LOGD^ZAA02GCOMM5(TO)" D ^ZAA02GPOP Q:X=""  Q:X[":EX"  S DT=X
 S Y="18,10\RHLSP3\3\LOG ON: "_$$LOGD(DT),Y(0)="\EX\@ZAA02GCOMMG@(""LOG"",DT)\$$LOGT^ZAA02GCOMM5(TO)" D ^ZAA02GPOP Q:X=""  Q:X[":EX"
 Q
...
LOGT(TM) S K=$S($D(@ZAA02GCOMMG@("LOG",DT,TM))#2:^(TM),1:""),TM=TM\60,TM=$E(TM\60+100,2,3)_":"_$E(TM#60+100,2,3) Q TM_"  "_K
 ;
...
RECEIVEK W !!!,"Ready to RECEIVE file(s) via KERMIT",!!,"You may start your transfer at any time within the next 2 minutes",!!
 S MODEM=0,ER=0,KP="",GLOB="^ZAA02GKERMIT("_(+$H)_","_$P($H,",",2)_")",FGLOB=GLOB D INIT^ZAA02GCOMMA,REXE^ZAA02GCOMME
...
SENDK ; SEND/RECEIVE KERMIT FILES
 ...
 W A X @ZAA02G@("ECHO-ON") W !! S A="" F J=1:1 S A=$O(@ZAA02GCOMMG@("FILES",A)) Q:A=""  W J,?3,A,?20,$P(^(A),"`"),! S A(J)=A_","_$P(^(A),"`",2)
 ...
 S @ZAA02GCOMMG@("LOG",+$H,$P($H,",",2))="Download - "_$P(A(A),",")
 S GLOB="^ZAA02GKERMIT("_(+$H)_","_$P($H,",",2)_")",@GLOB@(0,"FILE",1)=$P(A(A),","),^(1,0)=$P(A(A),",",2),MODEM=$I,SFL=0,KP="" D SENDGLO^ZAA02GCOMMD
