ZAA02GCOMM6 ;PG&A,ZAA02G-COMM,1.36,MODEM TRANSFER (SCRIPT CONTROL);29DEC95 3:03P;;;09FEB98  09:55 [ 02/17/98   9:40 AM ]
 ;PG&A, 1984
 ;
RUN I $O(@ZAA02GCOMMG@("SCR",""))="" W !!,"NO SCRIPTS FOUND - PRESS RETURN" R A#1 Q
 D EDT1^ZAA02GCOMM7,EDS^ZAA02GCOMM7 I X'="",$D(YY(X))  S SCR=YY(X) K YY,%1,%2,%3,%4,%5 S %IN=1 D RUNX
 K YY,%IN,QT Q
RUNX D:'$D(ZAA02G) INIT^ZAA02GDEV D ZAA02GSINT I '$D(LTERMON) S LOS=@ZAA02G@("OS") D OSS^ZAA02GCOMM2
...
RUNQ S CNT=10,DELAY=0,TRACE=0,(C,ST)="",ER=0 S:'$D(QT) QT=1 K CPI,FILTER S:'$D(MODEM) MODEM=0,MFL=0
 ...
 Q
O0 D TERMON^ZAA02GCOMM1
ODNLD D O3 I $D(WPDN) D DNLD
 D TERMOF^ZAA02GCOMM1 K D,DV,ZAA02G("TR"),CNT,DELAY,TRACE,ST,TR,XT,LAB,SCR,TRM,FILA,FILTER,CPZ Q
O3 S SCRL=.03 K LAB F I=1:1 S L=SCRL,SCRL=$O(@ZAA02GCOMMG@("SCR",SCR,SCRL)) Q:SCRL=""  I ^(SCRL)?1.AN1":" S LAB($P(^(SCRL),":"))=L
 S SCRL=.03 F I=1:1 S SCRL=$O(@ZAA02GCOMMG@("SCR",SCR,SCRL)) Q:SCRL=""  S TXT=^(SCRL) I TXT'="" I TXT'?1.AN1":",$E(TXT,1,3)'="REM",$E(TXT)'=";" D O4 Q:SCRL=999999
 Q
...
O52 X B Q
O53 G SKERMIT^ZAA02GCOMMA
O54 G RKERMIT^ZAA02GCOMMA
O55 W:$D(ZAA02G("F")) ZAA02G("F") Q
O56 G SASCII^ZAA02GCOMM8
O57 G RASCII^ZAA02GCOMM8
O59 S SCRL=99999 Q
O60 D O601 Q
...
O64 D BAUD^ZAA02GCOMM7 Q
O65 Q:B=""  S D=B I D?1.N1"."1.N1"."1.N1"."1.N S ip=D,port=23 D IP^ZAA02GCOMM1 S (D,DV)=MODEM D TERMON^ZAA02GCOMM1 Q
 O D::0 E  W !,"<<<DEVICE NOT AVAILABLE>>>",! S SCRL=999999 Q
 S (MODEM,DV)=D,A=ZAA02G(1),ZAA02G(1)=0 D LON^ZAA02GCOMM1 Q
O66 S:B?1.N ER=B Q
O67 H +B Q
O68 S ER=1 F K=1:0:CNT U MODEM R A:1 S:'$T K=K+1 X TRM S E=A X XT S A="" X:QT "U 0 W E R A:0 X T" Q:A[TR  S C=$S($L(C)+$L(E)<255:C_E,$L(E)>100:E,1:$E(C,200,255)_E) I C[B S ER=0 Q
 ...
 G O42E
O69 D SGLOBAL^ZAA02GCOMM7 Q
O70 D RGLOBAL^ZAA02GCOMM7 Q
O71 D SROUTINE^ZAA02GCOMM7 Q
...
O44 U:QT 0 S CP="@ZAA02GCOMMG@(""XFR"","""_$I_""")" S:B]"" CP=B I '$D(CPI) U:QT 0 K @CP S CPI=10,CPC="" F J=0:2:31 S CPC=CPC_$C(J,J+1)
...
TIME S A=$H F J=1:1 Q:$H'=A
 ...
 S TIM=J*20 Q
 ;
ZAA02GSINT S ZAA02GCOMMG=$S($D(^%ZAA02GSEND)#2:^%ZAA02GSEND,1:"^ZAA02GSEND") Q
WP ; MACRO ENTRY - #S SCR="X" G WP^ZAA02GCOMM6
 S REFRESH=1,KK=0,A=.03 F J=1:1 S A=$O(@ZAA02GWPD@(A)) Q:A=""  I $P(^(A),XF,4)'?." " S WPDN=A
 D ZAA02GSINT,WP1 S MC="" Q:'KK
 U 0 K SVSB S ZAA02GINS="@ZAA02GCOMMG@(""XFR"","""_$I_""")"
 ...
 Q
WP1 N (ZAA02GCOMMG,WPDN,ZAA02G,ZAA02GP,KK,SCR,ZAA02GWPD,DIR,DOC) D @$S($D(SCR):"RUNX",1:"RUN") S KK=$S($G(CPI):CPI-10,1:0) K SCR U 0 S A=ZAA02G(1),ZAA02G(1)=0 X @ZAA02G@("TERM-ON") S ZAA02G(1)=A Q
