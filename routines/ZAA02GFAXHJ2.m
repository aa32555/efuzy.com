ZAA02GFAXHJ2 ;PG&A,ZAA02G-FAX,1.36,MH ENCODING-2BYTE MODE;26JUL94  22:27 [ 08/22/96   9:33 AM ]
 ;Copyright (C) 1992, Patterson, Gray and Associates Inc.
 ;
LINE S TXW=1728-MG-CVR D SCAN I TXT'[XS S TXT=$TR(TXT,CA,CB) D LINEL F TXR=1:1:$S($D(SC):19,1:FR) D LN1 H 0
 I TXT[XS D VIDEO S TXL=$L(TXT) D LINEE F TXR=1:1:$S($D(SC):19,1:FR) D LN H 0
...
CF S FNT=FNT(D),D=D+1,NTX=$TR(TXT,AL,^ZAA02GFAXF(FNT,.1,TXR)),NTY=$TR(TXT,AL,^ZAA02GFAXF(FNT,.2,TXR)) K TR S:$D(^ZAA02GFAXF(FNT,.4)) TR=^(.4) Q
 ;
LN1 S LX="",L=EOL,W=MG I TXR<17 S NTX=$TR(TXT,AL,^ZAA02GFAXF(FNT,.1,TXR)),NTY=$TR(TXT,AL,^ZAA02GFAXF(FNT,.2,TXR))
 E  S NTX=$TR($J("",TXL),AL,^ZAA02GFAXF(FNT,.1,1)),NTY=$TR($J("",TXL),AL,^ZAA02GFAXF(FNT,.2,1))
 F I=1:1:TXL S X=^ZAA02GFAXF(MF,.3,$A(NTY,I)*256+$A(NTX,I)),W=W+X S:$D(SC(I)) W=W-X,X=$S($P(SC(I),",",TXR):^ZAA02GFAXJ(0,$P(SC(I),",",TXR)),1:32),W=W+X,I=I+1 I X["," S L=L_$S(W<32:wht(W),1:^ZAA02GFAXC(.1,W))_$P(X,",",2),W=$P(X,",",3) D:$L(L)>32 LSX
