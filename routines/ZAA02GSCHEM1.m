ZAA02GSCHEM1 ;PG&A,ZAA02G-SCHEMA,1.2,Database Dictionary Edit;10MAY89 12:49A
 ;Copyright (C) 1984, Patterson, Gray & Associates, Inc.
 ;
BEG S %R=3,%C=1 W @ZAA02GP,ZAA02G("CS")
 S B="" I $D(^ZAA02GSCHEMA(S1)) S B=^(S1)
 S (OLD,NEW)=B,%R=4,%C=2 W @ZAA02GP,ZAA02G("CS"),ZAA02G("LO"),"Dictionary Item: ",ZAA02G("HI"),S1 I $D(^(S1))=0 W "    (new code)"
DIS F I=1:1:16 S T=$P($T(@I),";",2,999),P=$P(T,"\",2),%R=$P(T,"\",3),%C=$P(T,"\",4),X=$P(T,"\",5) W @ZAA02GP,ZAA02G("HI"),$J(I_") ",4),ZAA02G("LO"),X,$E("....................................",1,28-%C-$L(X)) D:$P(NEW,"`",P)]"" DSPLY
 ;
SEL S %R=24,%C=5 W @ZAA02GP,ZAA02G("LO"),"Type ",ZAA02G("HI"),"E",ZAA02G("LO"),"dit item, ",ZAA02G("HI"),"D",ZAA02G("LO"),"elete code, ",ZAA02G("HI"),"L",ZAA02G("LO"),"ist Forms, ",ZAA02G("HI"),"R",ZAA02G("LO"),"ecompile Forms, or ",ZAA02G("HI"),"Q",ZAA02G("LO"),"uit ",ZAA02G("HI")
 R A#1 S:A?1L A=$C($A(A)-32) I A=""!("DQLER"'[A) W *7,*8 G SEL
 I A="Q" S J=-1 K NEW,OLD,T Q
 I A="D" G DELETE
 I A="R" S ACOMPILE=1 G COMPILE^ZAA02GSCHEM2
 I A="L" G LIST^ZAA02GSCHEM3
SELECT S %R=24,%C=1 W @ZAA02GP,ZAA02G("CL"),$J("",20),ZAA02G("HI"),"Here to Continue ==>"
 X ^ZAA02G("ECHO-OFF") F J=1:1:16 S T=$P($T(@J),";",2,999),%R=$P(T,"\",6),%C=$P(T,"\",7),S=1,RX=0 W @ZAA02GP D RD S:'S $P(NEW,"`",P)=X I RX=1 S J=J-2 Q:J<0
 I J<0  K NEW,OLD,T Q
 S %R=24 W @ZAA02GP,"wait" 
FILE X ^ZAA02G("ECHO-ON") I NEW'=OLD S ^ZAA02GSCHEMA(S1)=NEW I $P(NEW,"`",3,18)'=$P(OLD,"`",3,18) D UPDATE^ZAA02GSCHEM2 Q
 K T,NEW,OLD Q
 ;
DELETE W @ZAA02GP,ZAA02G("CL") I $D(^ZAA02GSCHEMA(0,S1))>1 W ZAA02G("HI"),"Sorry, in use!" R A:2 I 1
 E  W ZAA02G("LO"),"Do you want to delete this code (",ZAA02G("HI"),"Y or N",ZAA02G("LO"),") ",ZAA02G("HI") R A#1 I A'="","Yy"[A S J=1 K ^ZAA02GSCHEMA(S1) W "...deleted" H 1
 S J=1 K NEW,OLD,T Q
 ;
DSPLY S %R=$P(T,"\",6),%C=$P(T,"\",7) W @ZAA02GP,ZAA02G("HI"),$P(NEW,"`",P) Q
