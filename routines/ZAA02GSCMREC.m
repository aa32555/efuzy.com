ZAA02GSCMREC ;PG&A,ZAA02G-MTS,1.20,CATEGORIES I-V - COMBINED RECOMM;;;;25MAR99  17:47
...
POP1 S (S,Z,W,SY,TS)=0 F I=1.1,2.1 S TS(I)=""
 S T="STUDY:,11,11;Bilateral,,25,TSB;Left,,39,TSL;Right,,49,TSR" D TT
 S DUP="LRB;123450"
 S T="CATEGORY:,13,11;1 (negative assessment - routine follow up),,25,ABN|RCN;2 (benign assessment - routine follow up),14,25,ABB|RCN;3 (benign appearing - 6 month follow up),15,25,ABP|RCF" D TT
 S T=";4 (suspicious abnormality - biopsy),16,25,ABS|RCB;5 (unequivocally malignant - biopsy),17,25,ABM|RCB;0 (Incomplete evaluation - follow up),18,25,ABA" D TT
...
TT2 S A=$F($TR(C,"ABCDEFGHIJKLMNOPQRSTUVWXYZ123450","\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\"),"\"),L=$E(C,A-1),SS(I_L)=M_$E(C,1,A-2)_ZAA02G("HI")_L_ZAA02G("LO")_$E(C,A,255),L=I_L S:SY SY(L)=""
 ...
 Q:'$D(BLD)  S $X=0,A=$TR($P(D,",",4),"|",";"),@ZAA02GSCMD@("XDIR",SCr,1,L)=A_","_C W:'$D(@ZAA02GSCMD@("DICT",3,$P(A,";"))) "not in DICT-",A I A]"" S @ZAA02GSCMD@("XDIR",SCr,2,$P(A,";"))=L_$S(A[";":","_$P(A,";",2),1:"") Q
 Q
HLP S C=X  S EDIT=$G(EDIT)_"A",TYPSS=TYPS,TYPS="" G END
 ;
RD R A#1 I A="" X ZAA02G("T") S ZF=$E(XX,$F(XX,ZF)) Q
RD2 S:A?1L A=$C($A(A)-32) S ZF="",L=$C(C+64)_A I '$D(SS(L)) S:"AB"[A A=$C($A(A)+32),L=$C(C+64)_A I '$D(SS(L)) W *7 Q
