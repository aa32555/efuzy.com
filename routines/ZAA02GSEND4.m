ZAA02GSEND4  ;;2014-07-10 17:23:46
ZAA02GSEND4 ;PG&A,ZAA02G-SEND,1.36,PHONELIST;4MAR94 12:28P;;;24NOV97  12:56
 ;Copyright (C) 1986, Patterson, Gray and Associates, Inc.
DIAL K M ; I $D(@ZAA02GSENDG@("DIAL"))=0 W *13,"No numbers are stored in ",ZAA02GSENDG S M="" H 2 Q
 S %T=ZAA02GSENDG_"(""DIAL"")"
...
D6 W ZAA02G("F"),!!,"Please enter/edit the following information:" S Y="NAME\PHONE #\LOGON INFO (optional)\TYPE OF SYSTEM (optional)\ZAA02G-SEND SCRIPT (optional)"
...
D8 R !,"Enter selection to move ",A I $D(A(A))=0 W " ???" G D1
 W !,"What is the new heading of ",A(A)," - " R B S @ZAA02GSENDG@("DIAL",B,A(A))=@%T@(A(A)) G D1
 ;
DE K A,MM S M="" Q
 ;
HEAD W:ZAA02G["VT" *15 W ZAA02G("F") S %R=1,%C=2 I $D(ZAA02G("a"))=0 W ZAA02G("LO"),@ZAA02GP,ZAA02G("RON")," P ",ZAA02G("RT")," G ",ZAA02G("RT")," & ",ZAA02G("RT")," A " S %C=69 W @ZAA02GP,"  ZAA02G-SEND  ",ZAA02G("ROF"),!
...
UCI S C=31 I LOS="MSM" W !! S C=0 F V=0:1:5 F U=1:1:30 S B=$ZU(U,V) I B]"" W ?5,C+1,?8,B,! S C=C+1,A(C)=V*32+U
 R !!,"Enter new UCI number - ",U G:U="" T4 I U>0,U'>C X $S(LOS="PSM":"ZC U",LOS="MSM":"V 2:$J:A(U):2",1:"I 0") K A G T4
...
PR I $D(TP) C TP W !!,"PRINTING TURNED OFF - PRINTER ",TP," CLOSED" K TP S XT=$P(XT," U TP W B")_$P(XT," U TP W B",2) H 2 G T4
 ...
 ;
RDIAL D ZAA02GSEND4 G:M="" T4 U 0 I $D(@ZAA02GSENDG@("DIALINIT",MODEM))#10 S SCR=^(MODEM),QT=1 D RUNQ^ZAA02GSEND6 H 1
 I $P(M,"\",4)]"" S SCR=$P(M,"\",4) I $D(@ZAA02GSENDG@("SCR",SCR))#10 D RUNQ^ZAA02GSEND6 H 1 U 0 G T4
 U MODEM W:$P(M,"\")]"" $S($D(@ZAA02GSENDG@("DIALPREFIX",MODEM))#10:^(MODEM),$D(@ZAA02GSENDG@("DIALPREFIX"))#10:^("DIALPREFIX"),1:"ATDT"),$P(M,"\"),*13 U 0 D ENTRY^ZAA02GSEND1 U 0 G T4
 ;
CMP W !! D OSP^ZAA02GSEND2 W "Local system=",LOS," Remote system=",OS,!
 S DIRECT="L",TYPE="R" D UTIL^ZAA02GSEND X LRSEL S %N=$O(@UTIL@(0))'=""
 G T4:%N=0 S %JJ=%J U MODEM F  R A:1 I  U 0 Q
 W ! R "Long or Short Listing - (LS) ",%W,!
 W !! S %RR=0
ROUT S:0 $ZE="G ERROR^ZAA02GSEND4" S CL=ZAA02G("CL"),LINE=$P($T(LINE4),";",2,255),LINE1=$P($T(LINE1),";",2,255),LINE2=$P($T(LINE2),";",2,255),LINE6=$P($T(LINE6),";",2,255)
 I LOS="PSM" S LINE=$P($T(LINE),";",2,255)
 I OS="PSM" S LINE1=$P($T(LINE3),";",2,255),LINE2=$P($T(LINE5),";",2,255)
 S XR=$P(XR,"A")_"$P(%L,"""" """",2,255)"_$P(XR,"A",2),LXR=$P(LXR,"A")_"$P(%L,"" "",2,255)"_$P(LXR,"A",2),LINE=LINE_LXR,LINE1=LINE1_XR_""",*13"
 S DV=MODEM X:DV'=56 ^ZAA02G("ECHO-OFF") U 0
 X $P($T(SEARCH)," ",2,256) W *13,"           ",!,"USE ""."" SYNTAX WITH %RSEL",!,"Press RETURN to continue " R %C#1
EXIT K %,%C,%F,%I,%JJ,%K,%L,%N,DV,%P,%RR,%SS,%W,%X,%SF,LIN,MORE,DO,%PU G T4
SEARCH F %=0:0 S %RR=$O(^UTILITY(%JJ,%RR)),%F=0 Q:%RR=""  W *13,%RR,?12,CL X LINE1,LINE S B="" X "F %=1:1:999 R *A:2 Q:A<0  I A=1 R A#3:2 R:A>0 B#A:2 Q" U 0 K:%T=+B ^UTILITY($J,%RR) I %T'=+B X LINE6
LINE6 ;W $S(B["*"&(+B=0):"missing",B["*":"different",1:"error"),"  ",B,! I %W="L" X LINE2
LINE ;S %T=0 ZL @%RR F %N=1:1 S %L=$T(+%N) Q:%L=""  S %T=%T+
LINE4 ;S %T=0 F %N=1:1 S %A="%L=$T(+"_%N_"^"_%RR_")",@%A Q:%L=""  S %T=%T+
LINE1 ;U DV W "S %T=0 F %N=1:1 S %A=""%L=$T(+""_%N_""^",%RR,")"",@%A W:%L="""" $C(1),$E(1001+$l(%T)+"_$L(%RR)_",2,4),%T,""*"_%RR_""" Q:%L=""""  S %T=%T+
LINE3 ;U DV W "S %T=0 ZL "_%RR_" F %N=1:1 S %A=""%L=$T(+%N)"",@%A W:%L="""" $C(1),$E(1001+$L(%T),2,4),%T,""*"" Q:%L=""""  S %T=%T+
LINE2 ;S %T=0 ZL @%RR S %L=$T(+1) U DV W "S %A=""%L=$T(+""_1_""^",%RR,")"",@%A W %L,$C(13)",*13 R A,B,C U 0 W "LOCAL:",!,%L,!,"REMOTE:",!,B,! R CCC
LINE5 ;S %T=0 ZL @%RR S %L=$T(+1) U DV W "S %A=""%L=$T(+1)"",@%A W %L,$C(13)",*13 R A,B,C,C U 0 W "LOCAL:",!,%L,!,"REMOTE:",!,C,! R CCC
