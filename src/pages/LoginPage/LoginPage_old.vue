    ...
    <div class="bx--grid bx--grid--full-width landing-page imgfill" v-if="!checkingLogin">
      <div class="bx--row landing-page__r2">
        <div class="bx--col bx--no-gutter ">
          <q-no-ssr>
          <cv-modal id="modal" style="background-image:linear-gradient(#48c6ef,#6f86d6);z-index:1" class="imgfill" :size="'small'" :close-aria-label="'login'" :visible="true" :auto-hide-off="true" ref="modal">
...
<template slot="secondary-button">
  ...
  <button v-if="signupScreen" role="button" class="cv-button bx--btn bx--btn--secondary" :style="'margin-top:16px;padding:0px;height:90%'" @click.prevent="showLogin()">
          ...
          v-if="appPickerModal"
  :size="'xs'"
  :visible="appPickerModal"
  :auto-hide-off="false">
  <template slot="title">Select App</template>
  <template  slot="content">
  <FUZDiv :colLG="12">
     <cv-select
  :label="''"
  @change="handleAppPicked"
  :disabled="false"
  v-model="pickedApp"
  :helper-text="'Please select an app to login'">
    <cv-select-option v-for="app in registeredApps" :key="app.value" :value="app.value"  >{{app.label}}</cv-select-option>
  </cv-select>
...
// import FUZMultiselectInput from '../../FUZ/FUZMultiselectInput'
...
export default {
  data () {
    return {
      ...
      bodyWidth: 0,
      resized: false,
      loaderPath: 'animations/login.json',
      animations: {
        Login
      },
      appPickerModal: false,
      pickedApp: '',
      registeredApps: []
      ...
      setTimeout(async () => {
        try {
          ...
          const res = await axios({
            url: 'https://www.hairsalonsystems.com/apps/login',
            method: 'post',
            data: {
          ...
          */
          ...
          if (typeof status === 'object') {
            this.registeredApps = status.apps
            this.$nextTick(() => {
              this.appPickerModal = true
            })
            return
          } else {
            if (status !== 'OK') {
