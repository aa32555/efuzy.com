...
export default async (routine, data) => {
  ...
  try {
    ...
    const result = await axios({
      url: 'https://www.efuzy.com/salon/api',
      method: 'post',
      headers: {
        auth: (store.state.app && store.state.app.session && store.state.app.session.session) || '',
        jwt: (store.state.app && store.state.app.session && store.state.app.session.jwt) || ''
      },
      data: { routine, data }
    })
